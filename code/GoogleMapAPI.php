...
class GoogleMapAPI extends ViewableData {
   ...
   * you want to use JS Minification.
   ...
   /**
    * Use a custom HTML info window
    *
    * @var boolean
    */
   var $custom_infobox = false;
      

   /**
    * The configuration for the custom infobox. Valid keys:
    * width: The width of the infobox
    * height: The height of the infobox
    * offsetVertical: The vertical offset of the box from the marker
    * offsetHorizontal: The horizontal offset of the box from the marker
    */
   var $custom_infobox_config = array (
   		'width' => 200,
   		'height' => 100,
   		'offsetVertical' => -275,
   ...
   */
    ...
    function enableCustomInfobox($config = array ()) {
        $this->custom_infobox = true;
   ...
   */
   ...
   function addKMLOverlay($file){
    ...
    function getHeaderJS() {
      $_headerJS = "";
      if( $this->mobile == true){
        Requirements::insertHeadTags("<meta name='viewport' content='initial-scale=1.0, user-scalable=no' />");
      }
      if(!empty($this->_elevation_polylines)||(!empty($this->_directions)&&$this->elevation_directions)){
        ...
        Requirements::customScript("google.load('visualization', '1', {packages: ['columnchart']});");
      }
      $s = 'http://maps.google.com/maps/api/js?sensor=';
      $s .= ($this->mobile==true)?'true':'false';
      Requirements::javascript($s);
      if($this->marker_clusterer){
        Requirements::javascript('mappable/javascript/clusterer.js');
      }
      if($this->custom_infobox) {
          Requirements::javascript('mappable/javascript/infobox.js');
      }    
  ...
  */
     ...
     */
    function getOnLoad() {
        Requirements::customScript('window.onload=onLoad'.$this->map_id.';');
    }
    ...
    Requirements::customScript($_script);
  }
   ...
   */
   function getAddOverlayJS(){
    ...
    function getCreateMarkerJS() {
      $functionBody = $this->custom_infobox ? "openCustomInfoWindow(map, new_marker, html);" : "openInfoWindow(map, new_marker);";
      ...
      $_output = "
         ...
         function openCustomInfoWindow(map, marker, html) {
			if(openWindow) openWindow.setMap(null);
            var infoBox = new InfoBox({latlng: marker.getPosition(), map: map, content: html, width: ".$this->custom_infobox_config['width'].", height: ".$this->custom_infobox_config['height'].", offsetVertical: ".$this->custom_infobox_config['offsetVertical'].",offsetHorizontal: ".$this->custom_infobox_config['offsetHorizontal']."});
            map.setCenter(marker.getPosition());
            openWindow = infoBox;	         
         }
         ...
         function createMarker(map, point, title, html, icon, icon_shadow, sidebar_id, openers){
          ...
          ".(($this->info_window)?"
              ...
              google.maps.event.addListener(new_marker, '".$this->window_trigger."', function() { $functionBody
          });
   ...
   */
   function getElevationMarkerJS(){
    ...
    function getMap() {
        ...
        return "<script type='text/javascript'>$_output</script>";
    }
